  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{shopName} · {data.length} 位客户{isDemo && <span className="ml-2 text-xs text-amber-400">(演示)</span>}</p></div>
        <Button size="sm" variant="outline" onClick={function () { setShowConfig(true); }} className="h-8 gap-1 text-xs"><Settings className="h-3 w-3"/>阈值</Button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          return <Card key={i} className="border-border/40 bg-card/60"><CardContent className="p-3 text-center"><p className={"text-xl font-bold tabular-nums " + (s.c || "")}>{s.v}</p><p className="text-[10px] text-muted-foreground">{s.l}</p></CardContent></Card>;
        })}
      </div>

      {/* Customer Pyramid */}
      <Card className="border-border/40 bg-card/60"><CardContent className="p-3">
        <p className="text-[10px] font-semibold text-muted-foreground mb-2">客户金字塔</p>
        <div className="space-y-1">
          {pyramidData.map(function (l) {
            return <div key={l.name} className="flex items-center gap-2 text-[10px]">
      ...
      <Card className="border-border/40 bg-card/60"><CardContent className="flex flex-wrap items-center gap-1.5 px-3 py-2">
        <div className="relative flex-1 min-w-[100px]"><Search className="absolute left-2 top-1/2 h-3 w-3 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={function (e) { setSearch(e.target.value); }} placeholder="搜索客户..." className="h-7 pl-7 text-[10px]" /></div>
        <select value={filterSegment} onChange={function (e) { setFilterSegment(e.target.value); }} className="h-7 rounded border border-border/40 bg-background text-[10px] px-1"><option value="all">全部群体</option>{segments.map(function (s) { return <option key={s.segment} value={s.segment}>{s.emoji} {s.label}</option>; })}</select>
        <Button size="sm" variant="outline" onClick={function () { setShowMigration(!showMigration); }} className="h-7 text-[10px]">{showMigration ? "隐藏迁徙" : "迁徙矩阵"}</Button>
      ...
      {expandedId !== 0 && (function () {
        ...
        if (!c) return null;
        return <Card className="border-border/40 bg-card/60 border-l-2 border-l-cyan-500"><CardContent className="p-3 grid grid-cols-4 gap-2 text-[10px]"><p><span className="text-muted-foreground">首次购买:</span> {new Date(c.firstOrderDate).toLocaleDateString("zh-CN")}</p><p><span className="text-muted-foreground">平均客单:</span> {formatCny(c.avgOrderValue * EXCHANGE_RATE)}</p><p><span className="text-muted-foreground">国家:</span> {c.country || "—"}</p><p><span className="text-muted-foreground">LTV:</span> {formatCny(c.totalSpent * EXCHANGE_RATE)}</p></CardContent></Card>;
      })()}

      {/* Migration Matrix */}
      {showMigration && (
        <Card className="border-border/40 bg-card/60"><CardContent className="p-3">
          <p className="text-[10px] font-semibold text-muted-foreground mb-2">客户迁徙矩阵 (上期 → 本期)</p>
          <p className="text-[10px] text-muted-foreground">迁徙矩阵需要两期订单数据对比，请在真实模式下使用。</p>
        </CardContent></Card>
      )}

      {/* Trend Chart */}
      <Card className="border-border/40 bg-card/60"><CardContent className="p-3">
        <p className="text-[10px] font-semibold text-muted-foreground mb-1">群体趋势 (近12月·模拟)</p>
      ...
      <Card className="border-border/40 bg-card/60"><CardContent className="p-3 space-y-2">
        <p className="text-[10px] font-semibold text-muted-foreground">精准营销建议</p>
        {suggestions.map(function (sg, i) {
          var cfg = SEGMENT_CONFIG[sg.segment];
          return <div key={i} className="flex items-start gap-2 p-2 rounded bg-muted/10 text-[10px]">
        ...
        <div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-sm bg-card border border-border/40 rounded-xl shadow-2xl p-5 space-y-3">
              <h3 className="text-sm font-semibold">RFM 阈值配置</h3>
              {(["r","f","m"] as const).map(function (dim) {
                return <div key={dim} className="space-y-1">
                  <p className="text-[10px] font-semibold text-muted-foreground">{dim === "r" ? "R·Recency (天数≤)" : dim === "f" ? "F·Frequency (次数≥)" : "M·Monetary (金额≥ ¥)"}</p>
                  <div className="flex gap-1">
                    {thresholds[dim].map(function (v, i) {
                      return <Input key={i} type="number" value={v} onChange={function (e) { var arr = thresholds[dim].slice(); arr[i] = Number(e.target.value) || 0; setThresholds({ ...thresholds, [dim]: arr }); }} className="h-7 text-[10px] w-16" />;
                    })}
                  </div>
                </div>;
              })}
              <Button onClick={function () { setShowConfig(false); showToast("阈值已更新"); }} className="w-full h-9 text-xs bg-cyan-600 text-white">确认</Button>
            </div>
          </div>
        </div>
      )}
