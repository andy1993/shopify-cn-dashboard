  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-950">
      <div className="text-center max-w-md px-6">
        ...
        <p className="text-sm text-zinc-400 mb-2">{error.message || "未知运行时错误"}</p>
        {error.digest && (
          <p className="text-xs text-zinc-600 font-mono mb-4">Error ID: {error.digest}</p>
        )}
        <div className="flex gap-3 justify-center">
          <Button
            onClick={reset}
            className="h-9 bg-emerald-600 text-white text-sm hover:bg-emerald-500"
          >
            重试
          </Button>
          <Button
